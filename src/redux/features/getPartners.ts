...
export const getPartner = createAsyncThunk<returnType, paramsInput | null>(
  "partners/get",
...
export const deleteProperty = createAsyncThunk<returnType, comeType>(
  "property/delete",
...
export const withDrawProperty = createAsyncThunk<returnType, comeWithdrawType>(
  "property/withdraw",
...
export const UpdataExpiredDateProperty = createAsyncThunk<returnType, comeType>(
  "property/expireddata",
  ...
  extraReducers: (builder) => {
    builder.addCase(getPartner.fulfilled, (state, action) => {
      ...
      builder.addCase(deleteProperty.rejected, (state, action) => {
        state.loading = false;
        state.messageDelete = action.error.message
          ? action.error.message
          : "error";
      }),
      builder.addCase(withDrawProperty.fulfilled, (state, action) => {
        state.messageWithDraw = action?.payload?.message
          ? action.payload.message
          : "success";
      }),
      ...
      builder.addCase(withDrawProperty.rejected, (state, action) => {
        // state.loading=false
        state.messageWithDraw = action.error.message
          ? action.error.message
          : "error";
      }),
      builder.addCase(UpdataExpiredDateProperty.fulfilled, (state, action) => {
        state.messsageExpiredDate = action.payload.message
          ? action.payload.message
          : "";
      }),
      builder.addCase(
        UpdataExpiredDateProperty.rejected,
        (state, action) => {}
      ),
