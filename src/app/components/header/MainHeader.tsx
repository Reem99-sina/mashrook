...
export default function MainHeader() {
  ...
  const {
    ...
    user: userProfile,
    message,
  } = useSelector<RootState>((state) => state.register) as {
    token: string;
    user: userInfo;
    message: string;
  };
  ...
  return (
    <header className="border-b-2 mb-4 w-full bg-white">
      <div className="flex items-center justify-between">
        <div className="w-full self-center p-4 flex items-center gap-3">
          {token && (
            <div
            ...
            >
              <IoMdNotificationsOutline className="text-[28px] text-[#4B5563]" />
            </div>
          )}
