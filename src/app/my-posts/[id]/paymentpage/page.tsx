...
return   selectData?.propertyTypeDetails?.title + " " +pieces
  ...
  async function onSubmit(e:React.MouseEvent<HTMLButtonElement>){
    ...
    if(data?.method=="bank"){
      if(recipt){
        // dispatch(postMyAdvertise({property_id:selectData?.id,amount:100})).then((res)=>{console.log(res,"res")})
      }else{
      ...
      if(status==true){
      //  dispatch(postMyAdvertise({property_id:selectData?.id})).then((res)=>{console.log(res,"res")})
      }
    }
