...
export const OtherOfferCard: React.FC<ChatCardProps> = ({
  ...
  const save = useMemo(() => {
    return (otheroffer: dataReturnType) => {
      return otheroffer?.propertySaved
        ?.map(({ property_id }: saveElement) => property_id)
        .includes(otheroffer?.id);
    };
  }, []);
    ...
    <div className="mt-4 w-full border-2 border-[#E5E7EB] rounded-lg mb-4 flex flex-col p-4">
        ...
        <div className="flex flex-row gap-x-2 text-xs">
          <span
            className={`text-white text-right px-4 py-1 rounded-2xl ${
              otheroffer?.propertyPurpose?.title === "بيع"
                ? "bg-green-450"
                : "bg-orange-450"
            }`}
          >
      ...
      <div className="pt-1 mr-4 text-sm text-gray-700 mt-2">
        ...
        <div className="flex items-center  justify-start">
          <GoLocation />
          <p className="px-2">
            مدينة {otheroffer?.propertyLocation?.city}،{" "}
            {otheroffer?.propertyLocation?.district?.replace(/[\[\]\\"]/g, "")}
          </p>
        </div>
      </div>
